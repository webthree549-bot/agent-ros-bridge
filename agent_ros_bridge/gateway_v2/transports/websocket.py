...
class WebSocketTransport(Transport):
    ...
    async def _handle_client(self, websocket: "WebSocketServerProtocol"):
        ...
        if self.authenticator:
            ...
            token = self.authenticator.extract_token_from_query(
            ...
            if not auth_payload:
                logger.warning(
                    f"Authentication failed for client {client_id} - no valid token or API key"
                )
        ...
        try:
            async for message in websocket:
                try:
                        ...
                        if response:
                            ...
                            if self.rbac and response.telemetry:
                                filtered_data = self.rbac.filter_response(
                                    identity.roles, response.telemetry.data
                                )
                    ...
                    logger.warning(
                        f"Invalid JSON from {client_id}: {e}, raw: {repr(message[:100])}"
                    )
                    await websocket.send(
                        json.dumps(
                            {"error": "Invalid JSON", "type": "parse_error", "details": str(e)}
                        )
                    )
                except Exception as e:
    ...
    def _json_to_message(self, data: Dict[str, Any]) -> Message:
        ...
        header = Header(
            ...
            timestamp=datetime.fromisoformat(
                header_data.get("timestamp", datetime.utcnow().isoformat())
            ),
