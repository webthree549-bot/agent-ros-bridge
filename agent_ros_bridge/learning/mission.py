...
class AdvancedMissionPlanner:
    ...
    def replan_on_failure(
        self, original_plan: list[dict[str, Any]], failed_step: dict[str, Any]
    ) -> list[dict[str, Any]]:
    ...
    def optimize_task_order(
        self, tasks: list[dict[str, Any]], start_location: tuple[float, float]
    ) -> list[dict[str, Any]]:
        ...
        while remaining:
            # Find closest task
            closest = min(
                remaining, key=lambda t: self._distance(current_pos, t.get("location", (0, 0)))
            )
