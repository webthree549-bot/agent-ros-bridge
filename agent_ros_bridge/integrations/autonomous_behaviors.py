...
class MissionPlanner:
    ...
    def plan_exploration(self, area: ExplorationArea, 
                        strategy: str = "systematic") -> Mission:
    ...
    def plan_search(self, target_description: str, 
                   search_area: ExplorationArea) -> Mission:
        ...
        for step in mission.steps:
            ...
            if step.action == "navigate":
                enhanced_steps.append(MissionStep(
...
class AutonomousBehaviorManager:
    ...
    async def start_patrol(self, route: PatrolRoute, 
                          robot_id: str) -> str:
    ...
    async def _patrol_loop(self, behavior_id: str):
        ...
        while (behavior["status"] == BehaviorStatus.EXECUTING and 
               self._running):
    ...
    async def start_exploration(self, area: ExplorationArea,
                               robot_id: str) -> str:
...
def explore_autonomously(behavior_manager: AutonomousBehaviorManager,
                        area: Dict[str, float], 
                        robot_id: str = "default") -> str:
    """Start autonomous exploration.
...
def patrol_route(behavior_manager: AutonomousBehaviorManager,
                waypoints: List[Dict], 
                interval_minutes: int = 30,
                robot_id: str = "default") -> str:
    """Start patrol route.
    ...
    """
    route_waypoints = [
        Waypoint(x=w["x"], y=w["y"], z=w.get("z", 0), name=w.get("name"))
        for w in waypoints
    ]
