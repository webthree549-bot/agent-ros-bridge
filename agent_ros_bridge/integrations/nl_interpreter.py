...
class RuleBasedInterpreter:
    ...
    def _compile_patterns(self) -> List[Tuple[re.Pattern, callable]]:
        ...
        return [
            # Movement patterns
            (re.compile(r'(?:move|drive|go)\s+(forward|backward|back)\s+(?:for\s+)?([\d.]+)?\s*(?:m|meters?)?(?:\s+(\w+))?', re.I), self._handle_move),
            (re.compile(r'(?:move|drive|go)\s+(forward|backward|back)(?:\s+(\w+))?', re.I), self._handle_move_vague),
            ...
            # Navigation patterns
            (re.compile(r'(?:go|navigate|drive)\s+to\s+(?:the\s+)?(.+)', re.I), self._handle_navigate),
            ...
            # Fleet patterns
            (re.compile(r'list\s+(?:all\s+)?(?:available\s+)?robots', re.I), self._handle_list_robots),
    ...
    def _handle_status(self, match, context):
        """Handle status query."""
        return {
            "tool": "bridge_get_robot_status",
            "explanation": "Get robot status"
        }
