...
class SafetyManager:
    ...
    def register_policy(
        self, action: str, level: SafetyLevel, description: str = "", timeout: int = 30
    ):
        """Register safety policy for an action."""
        self.policies[action] = SafetyPolicy(
            action=action, level=level, description=description, timeout_seconds=timeout
        )
    ...
    async def request_confirmation(
        self, action: str, message: str, timeout: int = 30
    ) -> ConfirmationRequest:
        ...
        request = ConfirmationRequest(
            id=request_id, action=action, message=message, timestamp=datetime.now(), timeout=timeout
        )
    ...
    def trigger_emergency_stop(self, reason: str = ""):
        ...
        self.audit_log.append(
            {"type": "emergency_stop", "reason": reason, "timestamp": datetime.now().isoformat()}
        )
    ...
    def clear_emergency_stop(self):
        ...
        self.emergency_stop = False
        self.audit_log.append(
            {"type": "emergency_stop_cleared", "timestamp": datetime.now().isoformat()}
        )
