...
class RecoveryHandler:
    ...
    async def handle(self, anomaly: Anomaly) -> RecoveryResult:
        ...
        if self.recovery_count > effective_max:
            return RecoveryResult(
                success=False, action_taken="Max recovery attempts exceeded, aborting"
            )
        ...
        return RecoveryResult(success=False, action_taken=f"Unknown anomaly type: {anomaly.type}")
    ...
    async def _handle_stuck(self, anomaly: Anomaly) -> RecoveryResult:
        ...
        return RecoveryResult(success=True, action_taken="Backed up 0.5m and replanned path")
    ...
    async def _handle_deviation(self, anomaly: Anomaly) -> RecoveryResult:
        ...
        return RecoveryResult(success=True, action_taken="Re-localized robot and resumed execution")
    ...
    async def _handle_obstacle(self, anomaly: Anomaly) -> RecoveryResult:
        ...
        return RecoveryResult(
            success=True, action_taken="Waited for obstacle to clear, then replanned around"
        )
    ...
    async def _handle_timeout(self, anomaly: Anomaly) -> RecoveryResult:
        ...
        return RecoveryResult(success=True, action_taken="Aborted execution and notified operator")
    ...
    async def _handle_collision(self, anomaly: Anomaly) -> RecoveryResult:
        ...
        return RecoveryResult(
            success=False, action_taken="Emergency stop triggered due to collision"
        )
    ...
    async def _handle_controller_error(self, anomaly: Anomaly) -> RecoveryResult:
        ...
        return RecoveryResult(
            success=False, action_taken="Controller error, manual intervention required"
        )
    ...
    async def execute_motion(
        ...
        if not plan or not plan.primitives:
            return ExecuteMotionResult(
                success=False, status="FAILED", error_message="No plan to execute"
            )
        ...
        try:
            ...
            for i, primitive in enumerate(plan.primitives):
                ...
                # Execute primitive
                primitive_success = await self._execute_primitive(primitive, progress_callback)
    ...
    async def _execute_primitive(
        ...
        while elapsed < test_duration:
            ...
            anomaly = self.detect_anomaly(
                ...
                expected_duration=(
                    self.execution_state.plan.expected_duration
            ...
            if progress_callback:
                ...
                overall_progress = self.execution_state.current_primitive_index / len(
                    self.execution_state.plan.primitives
