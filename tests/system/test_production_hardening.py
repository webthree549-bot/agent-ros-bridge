...
class TestLLMParserHardening:
    ...
    def test_llm_invalid_api_key(self):
        ...
        try:
            ...
            with patch.object(parser, '_call_openai', side_effect=Exception("Authentication failed")):
                result = parser.parse("go to kitchen")
        ...
        try:
            ...
            for lang_code, lang_data in parser.LANGUAGE_PATTERNS.items():
                ...
                for intent in required_intents:
                    assert intent in lang_data.patterns, \
                        f"{lang_code} missing {intent} patterns"
                    assert len(lang_data.patterns[intent]) > 0, \
