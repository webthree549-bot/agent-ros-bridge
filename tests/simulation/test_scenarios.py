...
class TestScenarioFiles:
    ...
    def test_scenario_files_exist(self, scenarios_dir):
        ...
        assert (
            len(scenario_files) >= 10
        ), f"Expected at least 10 scenarios, found {len(scenario_files)}"
    ...
    def test_scenarios_have_success_criteria(self, scenarios_dir):
        ...
        for scenario_file in scenario_files:
            ...
            assert (
                "success_criteria" in scenario
            ), f"{scenario_file.name}: missing 'success_criteria'"
    ...
    def test_robot_references_valid_models(self, scenarios_dir):
        ...
        for scenario_file in scenario_files:
            ...
            for robot in scenario.get("robots", []):
                ...
                assert model_path.exists(), f"{scenario_file.name}: model '{model}' not found"
