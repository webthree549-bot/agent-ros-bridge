...
pytestmark = [
    pytest.mark.e2e,
    pytest.mark.skipif(
        os.system("which ros2 >/dev/null 2>&1") != 0,
        reason="ROS2 not installed"
    ),
    pytest.mark.skipif(
        os.system("which gz sim >/dev/null 2>&1") != 0,
...
class TestOpenClawE2E:
    ...
    def test_e2e_navigate_via_openclaw(self, ros_bridge, gazebo_sim):
        ...
        try:
            ...
            while time.time() - start_time < 30.0:  # 30 second timeout
                ...
                if current_pose:
                    distance_moved = ((current_pose.position.x - initial_x)**2 + 
                                    (current_pose.position.y - initial_y)**2)**0.5
            ...
            final_distance = ((current_pose.position.x - (initial_x + 1.0))**2 + 
                            (current_pose.position.y - initial_y)**2)**0.5
            
            assert final_distance < 0.5, f"Robot didn't reach goal (distance: {final_distance:.2f}m)"
    ...
    def test_e2e_agent_memory(self, ros_bridge):
        ...
        parser.add_conversation_turn(
            "the kitchen is at position (5, 3)",
            "CONFIGURE"
        )
