...
class TestPackagingScript(unittest.TestCase):
    ...
    def test_package_script_exists(self):
        ...
        self.assertTrue(
            PACKAGE_SCRIPT.exists(),
            f"Package script not found: {PACKAGE_SCRIPT}"
        )
    ...
    def test_skill_packages_successfully(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                self.assertTrue(
                    zipfile.is_zipfile(output_file),
                    "Output is not a valid zip file"
                )
                ...
                with zipfile.ZipFile(output_file, 'r') as zf:
                    ...
                    for f in forbidden:
                        self.assertNotIn(
                            f,
                            files,
                            f"Package should not contain {f}"
                        )
