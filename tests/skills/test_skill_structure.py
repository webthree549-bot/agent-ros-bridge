...
class TestSkillStructure(unittest.TestCase):
    ...
    def test_skill_directory_exists(self):
        ...
        self.assertTrue(SKILL_PATH.exists(), f"Skill directory not found: {SKILL_PATH}")

    def test_skill_md_exists(self):
        """Test that SKILL.md exists."""
        self.assertTrue(SKILL_MD_PATH.exists(), f"SKILL.md not found: {SKILL_MD_PATH}")
    ...
    def test_no_extraneous_files(self):
        ...
        for filename in forbidden_files:
            ...
            self.assertFalse(file_path.exists(), f"Extraneous file found: {filename}")
    ...
    def test_skill_md_has_yaml_frontmatter(self):
        ...
        self.assertTrue(
            content.startswith("---"), "SKILL.md must start with YAML frontmatter (---)"
        )

        # Check for closing ---
        frontmatter_match = re.match(r"^---\s*\n(.*?)\n---\s*\n", content, re.DOTALL)
        self.assertIsNotNone(
            frontmatter_match, "SKILL.md must have valid YAML frontmatter delimiters"
        )
    ...
    def test_yaml_frontmatter_has_required_fields(self):
        ...
        self.assertIn("name", frontmatter, "Frontmatter must have 'name' field")
        self.assertIn("description", frontmatter, "Frontmatter must have 'description' field")

        # Check no other fields
        allowed_fields = {"name", "description"}
        extra_fields = set(frontmatter.keys()) - allowed_fields
        self.assertEqual(extra_fields, set(), f"Frontmatter has extra fields: {extra_fields}")
    ...
    def test_name_is_valid(self):
        ...
        self.assertTrue(len(name) <= 100, "Name should be concise (<= 100 chars)")
    ...
    def test_description_is_comprehensive(self):
        ...
        self.assertTrue(len(description) >= 50, "Description should be at least 50 characters")
        self.assertTrue(
            len(description) <= 500,
        ...
        found_keywords = [kw for kw in triggering_keywords if kw.lower() in description_lower]
    ...
    def test_skill_body_exists(self):
        ...
        body = parts[2].strip()
        self.assertTrue(len(body) > 0, "SKILL.md must have body content")
    ...
    def test_skill_body_under_500_lines(self):
        ...
        self.assertTrue(line_count <= 500, f"Skill body should be <= 500 lines, got {line_count}")
...
class TestSkillContent(unittest.TestCase):
    ...
    def test_has_robot_command_examples(self):
        ...
        command_keywords = ["move", "navigate", "rotate", "cmd_vel", "list_robots", "get_topics"]

        found = [kw for kw in command_keywords if kw.lower() in content.lower()]
        self.assertTrue(
            len(found) >= 2, f"Skill should have robot command examples. Found: {found}"
        )
    ...
    def test_has_ros1_ros2_differentiation(self):
        ...
        self.assertTrue(has_ros1 or has_ros2, "Skill should mention ROS versions")
    ...
    def test_has_safety_documentation(self):
        ...
        self.assertTrue(len(found) >= 1, f"Skill should document safety features. Found: {found}")
