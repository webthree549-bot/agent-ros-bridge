...
class TestSafetyValidation:
    ...
    async def test_rejects_unsafe_plan(self):
        ...
        target_pose = MagicMock()
        primitive = NavigateToPosePrimitive(target_pose=target_pose, max_velocity=5.0)  # Exceeds 1.0 limit
        
        result = await node.plan_motion(primitive)
        
        assert result.success is False
        assert "safety" in result.error_message.lower() or "velocity" in result.error_message.lower()
