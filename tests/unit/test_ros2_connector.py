...
with mock.patch.dict('sys.modules', {'rclpy': mock.MagicMock(), 'rclpy.node': mock.MagicMock(), 'rclpy.qos': mock.MagicMock()}):
...
class TestErrorHandling:
    ...
    async def test_ros2_not_available(self):
        ...
        with mock.patch("agent_ros_bridge.gateway_v2.connectors.ros2_connector.ROS2_AVAILABLE", False):
...
class TestTelemetryHandling:
    ...
    def test_telemetry_conversion_in_callback(self, robot):
        ...
        telemetry = Telemetry(
            topic="/sensor/temp",
            data=robot._ros_msg_to_dict(mock_msg),
            quality=1.0
        )
