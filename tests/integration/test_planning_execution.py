...
class TestSafetyValidatorIntegration:
    ...
    async def test_unsafe_plan_rejected_before_execution(self):
        ...
        assert "velocity" in plan_result.error_message.lower() or "safety" in plan_result.error_message.lower()
    ...
    async def test_safety_certificate_expires(self):
        ...
        assert "certificate" in exec_result.error_message.lower() or "expired" in exec_result.error_message.lower()
...
class TestAnomalyDetectionAndRecovery:
    ...
    async def test_stuck_detection_triggers_recovery(self):
        ...
        assert "back" in recovery_result.action_taken.lower() or "replan" in recovery_result.action_taken.lower()
    ...
    async def test_obstacle_detection_triggers_recovery(self):
        ...
        anomaly = monitor.detect_anomaly(
            ...
            obstacle_distance=0.3  # Less than threshold
        )
        ...
        assert recovery_result.success is True
        assert "wait" in recovery_result.action_taken.lower() or "replan" in recovery_result.action_taken.lower()
