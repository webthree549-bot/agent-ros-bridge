#!/bin/bash
# demo/greenhouse/scripts/gh_control.sh - Greenhouse control commands
...
if action == "status":
    ...
    print(f"ROS Bridge: {r.get('ros', 'unknown')}")
    print(f"Mock Mode: {r.get('mock', 'unknown')}")
    print(f"Handlers: {', '.join(r.get('registered_handlers', []))}")
    
    # Get greenhouse-specific status
    r = send_cmd({"action": "get_greenhouse_status"})
    if r.get('status') == 'ok':
        ...
        print(f"Fan: {actuators.get('fan', 'N/A')}")
        print(f"Valve: {actuators.get('valve', 'N/A')}")
