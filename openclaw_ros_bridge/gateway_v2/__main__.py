...
Universal robot gateway with multi-protocol support.
"""
...
from openclaw_ros_bridge.gateway_v2.core import OpenClawGateway
from openclaw_ros_bridge.gateway_v2.config import ConfigLoader, GatewayConfig
from openclaw_ros_bridge.gateway_v2.transports.websocket import WebSocketTransport
from openclaw_ros_bridge.gateway_v2.transports.grpc_transport import GRPCTransport
from openclaw_ros_bridge.gateway_v2.connectors.ros2_connector import ROS2Connector
from openclaw_ros_bridge.gateway_v2.plugins.greenhouse_plugin import GreenhousePlugin
...
def create_gateway_from_config(config: GatewayConfig) -> OpenClawGateway:
    """Create and configure gateway from config"""
    gateway = OpenClawGateway(config.__dict__)
    ...
    for name, transport_cfg in config.transports.items():
        ...
        if name == "websocket":
            ...
            gateway.transport_manager.register(transport)
    ...
    for name, connector_cfg in config.connectors.items():
        ...
        if name == "ros2":
            ...
            gateway.connector_registry.register(connector)
    ...
    for plugin_cfg in config.plugins:
        ...
        if plugin_cfg.name == "greenhouse":
            ...
            asyncio.create_task(gateway.plugin_manager.load_plugin(plugin))
            logger.info(f"Loaded greenhouse plugin")
        
        # Dynamic plugin loading from source would go here
        # if plugin_cfg.source:
        #     plugin = load_plugin_from_source(plugin_cfg.source)
        #     gateway.plugin_manager.load_plugin(plugin)
    
    return gateway


async def run_gateway(config: GatewayConfig):
    """Run the gateway"""
    setup_logging(config.log_level)
    
    logger.info("=" * 60)
    logger.info("  OpenClaw Gateway")
    logger.info("  Universal Robot Gateway")
    logger.info("=" * 60)
    ...
    async with gateway.run():
        ...
        for name, transport in gateway.transport_manager.transports.items():
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OpenClaw Gateway - Universal Robot Gateway",
...
Environment Variables:
  ...
  OPENCLAW_LOG_LEVEL    Logging level (DEBUG, INFO, WARNING, ERROR)
  OPENCLAW_WEBSOCKET_PORT  WebSocket port
  OPENCLAW_GRPC_PORT    gRPC port
  OPENCLAW_JWT_SECRET   JWT secret for authentication
        """
    )
    ...
    parser.add_argument(
        "--version", "-v",
        action="version",
        version="OpenClaw Gateway 2.0.0"
    )
    ...
    if args.demo:
        ...
        config.name = "demo_gateway"
        config.transports["websocket"].port = 8765
        config.plugins.append(config.config.__dataclass_fields__['plugins'].type[0](
            name="greenhouse",
            enabled=True
        ))
    else:
